...
def copyanything(i, study_img_pth):
    try:
        shutil.copytree(i, study_img_pth)
    except OSError as exc:
        if exc.errno == errno.ENOTDIR:
            shutil.copy(i,study_img_pth)
        else: raise

def get_milliseconds():
    milliseconds = int(round(time.time() * 1000))
    milli = str(milliseconds)[-4:]
    return milli
...
for i in output_dirs:
    ...
    print(templ_clean)
    #break
    if set(study_count).intersection(templ_clean):
        os.chdir(loc_img_pth)
        os.getcwd()
        ts = get_milliseconds()
        print(ts)
        basename="/study_"
        study_img_pth = ("".join([loc_img_pth, basename]) + ts)
        print(study_img_pth)
        new_study_pth.append(study_img_pth)
        #os.mkdir(study_img_pth, mode=0o777)
        copyanything(i, study_img_pth)
        key_dirs.append(i)
        print(i)
...
seruid = []
for i in new_study_pth:
    print(i)
    os.chdir(dcmtk_loc_split)
    seruid.append(subprocess.check_output(["dcmdump", "+P", "0020,000e", "+sd", i], encoding="utf-8"))

print(seruid)

seruid_num = []
for i in seruid:
    seruid_num.extend(re.findall(r'(?<=\[)[^]]+(?=\])', i)) # Get the Series instance UID'sonly inside the square brackets
print(seruid_num)


series_count = []
for x in seruid_num:
    if x not in series_count:
        series_count.append(x) #Sort out the number of unique STUID's
print(series_count)
len(series_count)
...
def get_seriesuid():
    milliseconds = int(round(time.time() * 1000))
    milli = str(milliseconds)[-5:]
    nine_spot = int(random.randrange(1, 300))
    twelve_spot = str(random.randrange(1, 10**29))
    seruid = (f'1.2.3.40.7.11.{milli}.1.{nine_spot}.33.111.{twelve_spot}')
    return seruid
...
for i in new_study_pth:
    for root, dirs, files in os.walk(i):
        for file in files:
            if file.endswith((".dco", ".dcm")):
                series_locs.append(os.path.join(root, file))

print(series_locs)

#for i in series_locs: # It appears we have this data already in new_study_pth variable
#    placeholder_series.append(os.path.dirname(i))
#print(placeholder_series)    

#for x in placeholder_series: # It appears we have this data already in new_study_pth variable
#    if x not in u_placeholder_series:
#        u_placeholder_series.append(x)
#print(u_placeholder_series)   
#len(u_placeholder_series)     

for i in series_locs:
    print(i)
    os.chdir(dcmtk_loc_split)
    temp_ser = (subprocess.check_output(["dcmdump", "+P", "0020,000e", "+sd", i], encoding="utf-8"))
    print(temp_ser)
    ser_clean = (re.findall(r'(?<=\[)[^]]+(?=\])', temp_ser))
    print(ser_clean)
    if ser_clean != tmp_ser_clean:
        seid= get_seriesuid()
        print(seid)
        seid_go = '(0020,000e)={}'.format(seid) 
        subprocess.check_call(["dcmodify", "-m", seid_go, i])
        print('Generated a new UID and Edited')
    else:
        old_seid_go = '(0020,000e)={}'.format(old_seid) 
        subprocess.check_call(["dcmodify", "-m", old_seid_go, i])
        print('Used existing UID')
    print(ser_clean)
    old_seid = seid
    print(old_seid)
    tmp_ser_clean = ser_clean
    print(tmp_ser_clean)
    break
    
    
    
    if set(ser_clean).intersection(series_count):
        if seid = '' then:
            get a new series instance UID, otherwise continue below
        seid = get_seriesuid()
        subprocess.check_call(["dcmodify", "-m", "-ma", name_go,  "-ma", mrn_go, edit_it])
    ser_clean = tmp_ser_clean
...
###########################################################################################################################################################
#This is the old code copy mechansim to search for DCM files and move them to the created Image Directory - before the addition of finding unique studies by their STUID
##############################################################################################################################################################
...
#with open(loc_fn, mode='r') as csv_file:
     ...
     #for row in csv_reader:
             ...
             #print(i)
             #We generate a random number between 1 and the number lines that we calculated the CSV to be
